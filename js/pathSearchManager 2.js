...
class pathStationManager {
    ...
    static search(stoneHole, targetHole) {
        ...
        let turntimes=0;
        while((currentScan[0].isThereDirectionToScan()||lastStation.isThereDirectionToScan())&&completedPaths.length<1&&turntimes<10000){
            turntimes++;
            ...
            if(!nextStation.isThereDirectionToScan()){
             ...
             stationNullOrStoned=pathStationManager.isStationNullOrStoned(nextStation);
            if(stationNullOrStoned){
                continue;
            }
